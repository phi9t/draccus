...
`bin/draccus-uv` is a **global one-shot** wrapper for `uv` operations inside the namespace (it sets `UV_EXTRA_OVERRIDES`, `UV_CACHE_DIR`, and the right Python path). The per-project `.venv` is a **persistent activated environment** for that project alone. Common pitfall: running `bin/draccus-uv pip install …` while inside an activated `.venv` is almost never what you want — `draccus-uv` does not know about your activated venv and may install into a different interpreter. Inside a project: `source .venv/bin/activate && uv pip install …`. Outside any project: `bin/draccus-uv …` for foundation-level uv probes.

### 7.3 `UV_EXTRA_OVERRIDES` is a constraint, not a pin
`scripts/uv_overrides.txt` lists the DO_NOT_SHADOW set as **resolver constraints** (read via `UV_EXTRA_OVERRIDES`). When `uv lock` runs, the resolver refuses to satisfy any direct or transitive request for `torch`/`jax`/`numpy`/`scipy`/`triton`/`nvidia-*` from PyPI. The user sees a resolution failure at lock time — long before a runtime shadowing. Bound by `bin/draccus-run` at `/opt/draccus/uv_overrides.txt`.
